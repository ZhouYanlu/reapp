...
export default {
  name: "ListContainer",
  mounted() {
    //   派发action 通过vuex发起request请求，将数据存储在仓库中
    // console.log('组件的mounted');
    this.$store.dispatch("getBannerList");
    // 在new Swiper实例之前，页面中结构必须得有（但是现在把new Swiepr实例放在mounted中是不行的）
    // 因为dispatch中涉及到异步语句，导致v-for遍历的时候结构还没有获取完全，结构还不完整
  ...
  /* watch: {
    bannerList: {
      handler(newValue, oldValue) {
        //   当你执行这个语句的时候就说明数据已经回来了，数组里面就已经有数据了
